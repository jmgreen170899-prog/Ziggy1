...
def test_smoke_get_endpoints():
    ...
    for path, methods in paths.items():
        ...
        if "{" in path:
            ...
            skipped.append({"path": path, "reason": "Has path parameters"})
            continue
        ...
        if any(keyword in path.lower() for keyword in ["/admin", "/private"]):
            skipped.append(
                {"path": path, "reason": "Appears to require authentication"}
            )
            continue
        ...
        try:
            ...
            if response.status_code >= 500 and response.status_code not in [501, 503]:
                ...
                tested.append({"path": path, "status_code": response.status_code})

        except Exception as e:
            errors.append({"path": path, "status_code": None, "error": str(e)})
...
def test_health_endpoints():
    ...
    for endpoint in health_endpoints:
        ...
        if response.status_code == 200:
            ...
            failed.append({"endpoint": endpoint, "status_code": response.status_code})
...
def test_representative_routes_no_500():
    ...
    for path, method in test_routes:
        try:
            ...
            errors.append({"path": path, "method": method, "error": str(e)})
