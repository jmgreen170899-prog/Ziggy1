...
def test_positions_order_independent_no_change():
    streamer = PortfolioStreamer(
        connection_manager=cast(ConnectionManager, SimpleNamespace())
    )
    # Seed last data
    ...
    changed = asyncio.run(
        streamer._has_portfolio_changed(current_portfolio, current_positions)
    )
    ...
    current_positions = [
        {
            "symbol": "AAPL",
            "quantity": 10,
            "current_price": 100.009,
            "market_value": 1000.005,
        },
    ]
