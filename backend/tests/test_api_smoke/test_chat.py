...
def test_chat_completion_basic(client):
    ...
    assert response.status_code in [200, 500, 501, 502, 503], \
...
def test_chat_completion_with_options(client):
    ...
    assert response.status_code in [200, 400, 422, 500, 501, 502, 503], \
...
def test_chat_completion_empty_messages(client):
    ...
    assert response.status_code in [400, 422], \
        "Empty messages should return 400 or 422"
...
def test_chat_provider_consistency(client):
    ...
    if health_response.status_code == 200 and config_response.status_code == 200:
        ...
        assert health_data["provider"] == config_data["provider"], \
...
def test_chat_endpoints_availability(client):
    ...
    if response.status_code == 200:
        ...
        assert len(chat_paths) >= 2, "Should have at least health and complete endpoints"
        
        # Check for required endpoints
        assert any("/chat/health" in p for p in chat_paths), \
            "Should have health endpoint"
        assert any("/chat/complete" in p for p in chat_paths), \
