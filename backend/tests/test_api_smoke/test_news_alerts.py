...
def test_news_deprecated_headwind_alias(client):
    ...
    if response.status_code == 200:
        ...
        assert "ticker" in data or "score" in data, "Should have sentiment structure"
...
def test_news_sentiment_caching(client):
    ...
    if response1.status_code == 200 and response2.status_code == 200:
        ...
        assert (
            data1["updated_at"] == data2["updated_at"]
...
def test_alerts_endpoints_availability(client):
    ...
    if response.status_code == 200:
        ...
        assert any("/alerts/status" in p for p in paths), "Should have alerts/status"
        assert any("/alerts/start" in p for p in paths), "Should have alerts/start"
        assert any("/alerts/stop" in p for p in paths), "Should have alerts/stop"

        # Check for news endpoints
        assert any("/news/sentiment" in p for p in paths), "Should have news/sentiment"
