...
def test_market_overview_default(client):
    ...
    if "asof" in data:
        assert isinstance(data["asof"], (int, float)), "asof should be numeric timestamp"
...
def test_market_breadth(client):
    ...
    if response.status_code == 200:
        ...
        has_indicators = any(key in data for key in [
            "advancers", "decliners", "ad_ratio", "new_highs", "new_lows",
            "advancing", "declining", "unchanged", "detail"
        ])
...
def test_market_risk_lite(client):
    ...
    assert response.status_code in [200, 503], \
        "Risk-lite should return 200 or 503"
    ...
    if response.status_code == 200:
        ...
        if "cpc" in data:
            cpc = data["cpc"]
            assert "ticker" in cpc or "last" in cpc, \
