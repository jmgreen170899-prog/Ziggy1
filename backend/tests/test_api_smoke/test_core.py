...
def test_core_health_detailed(client):
    ...
    assert "service" in details or "total_routes" in details, \
...
def test_rag_query_basic(client):
    ...
    assert response.status_code in [200, 501], \
        "RAG query should return 200 or 501"
...
def test_vector_store_reset(client):
    ...
    # Status: 200 if available, 501 if not
    assert response.status_code in [200, 501], \
        "Reset should return valid status"
...
def test_task_list(client):
    ...
    # Status: 200 if available, 501 if not
    assert response.status_code in [200, 501], \
        "Task list should return valid status"
...
def test_task_cancel(client):
    ...
    schedule_response = client.post("/api/tasks/watch", json={
        "topic": "test cancel",
        "cron": "0 0 * * *",
    })
    ...
    # Try to cancel
    if job_id:
        cancel_response = client.delete("/api/tasks", json={
            "job_id": job_id,
        })
...
def test_rag_citation_structure(client):
    ...
    if response.status_code == 200:
        ...
        for citation in citations:
            ...
            if "url" in citation:
                assert isinstance(citation["url"], (str, type(None))), \
...
def test_core_endpoints_response_models(client):
    ...
    if detailed.status_code == 200:
        data = detailed.json()
        assert "status" in data and "details" in data, \
