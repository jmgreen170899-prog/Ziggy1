        ...
        for file in files:
            ...
            try:
                ...
                if router_id in registered_router_ids:
                    logger.debug(f"Router from {module_path} already registered (by id), skipping")
                    continue
                ...
                try:
                    if prefix:
                        ...
                        logger.info(f"Auto-registered router from {module_path} with prefix {prefix}")
                    else:
                        app.include_router(router)
                        logger.info(f"Auto-registered router from {module_path} (no prefix)")
