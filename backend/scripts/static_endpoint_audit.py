...
class EndpointVisitor(ast.NodeVisitor):
    ...
    def _has_pragma_ok(self, node: ast.FunctionDef) -> bool:
        ...
        if node.lineno > 1:
            ...
            if 'pragma: placeholder-ok' in prev_line or 'pragma:placeholder-ok' in prev_line:
                return True
        
        # Check first line of function
        if node.lineno < len(self.source_lines):
            func_line = self.source_lines[node.lineno - 1].strip()
            if 'pragma: placeholder-ok' in func_line or 'pragma:placeholder-ok' in func_line:
                return True
    ...
    def _is_placeholder_return(self, stmt: ast.Return, func: ast.FunctionDef) -> bool:
        ...
        if isinstance(stmt.value, ast.Dict):
            ...
            user_params = [p for p in params if p not in ['self', 'request', 'req', 'response', 'resp']]
...
def audit_directory(root_dir: Path) -> List[EndpointIssue]:
    ...
    for py_file in root_dir.rglob("*.py"):
        ...
        if any(part in py_file.parts for part in ['__pycache__', '.pytest_cache', 'tests', 'test', 'migrations']):
            continue
...
def print_report(issues: List[EndpointIssue]) -> None:
    ...
    print()
    print("=" * 100)
    print("To mark an endpoint as intentionally placeholder, add: # pragma: placeholder-ok")
    print("=" * 100)
