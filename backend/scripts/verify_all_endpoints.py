...
def main():
    ...
    for route_file in route_files:
        ...
        for endpoint in endpoints:
            ...
            print(
                f"  [{model_icon}M {test_icon}T {doc_icon}D] {endpoint['method']:6} {full_path:40} ({endpoint['function']})"
            )
    ...
    print(
        f"Endpoints with response models: {endpoints_with_models} ({endpoints_with_models/total_endpoints*100:.1f}%)"
    )
    print(
        f"Endpoints with tests: {endpoints_with_tests} ({endpoints_with_tests/total_endpoints*100:.1f}%)"
    )
    print(
        f"Endpoints with docstrings: {endpoints_with_docs} ({endpoints_with_docs/total_endpoints*100:.1f}%)"
    )
    ...
    for finding in all_findings:
        if not finding["has_model"]:
            print(
                f"  - {finding['method']:6} {finding['path']:50} in {finding['file']}"
            )
            count += 1
            if count >= 10:
