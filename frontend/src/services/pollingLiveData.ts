 ...
 */
import type { Quote, NewsItem, Alert, TradingSignal, Portfolio } from '@/types/api';
...
class PollingLiveDataService {
  ...
  connect(callbacks: PollingLiveDataCallbacks = {}): void {
    ...
    console.log('🔌 PollingLiveDataService.connect() - using REST polling fallback');
    ...
    const poll = async () => {
      ...
      try {
        ...
        for (const quote of quotes) {
          ...
          if (!lastQuote || lastQuote.price !== quote.price || lastQuote.timestamp !== quote.timestamp) {
