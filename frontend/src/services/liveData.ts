 ...
 */
import type { Quote, NewsItem, Alert, TradingSignal, Portfolio, PortfolioPosition } from '@/types/api';
...
class LiveDataService {
  ...
  constructor() {
    ...
    const envMax = Number((process.env.NEXT_PUBLIC_WS_MAX_RECONNECT_ATTEMPTS || '0').trim());
    this.maxReconnectAttempts = Number.isFinite(envMax) && envMax >= 0 ? envMax : 0;
   ...
   */
  private sendWhenOpen(socket: WebSocket | null, data: Record<string, unknown>, endpoint: string): void {
    try {
      if (!socket) {
        console.warn(`🔕 ${endpoint}: no socket instance available to send message`);
        return;
      }
      ...
      if (socket.readyState === WebSocket.CONNECTING) {
        ...
        socket.addEventListener('open', () => {
          try {
            ...
            console.error(`❌ Failed to send on ${endpoint} after open:`, err);
          }
        }, { once: true });
        return;
      }

      console.warn(`🔕 ${endpoint}: socket not open (state ${socket.readyState}), dropping message`);
    } catch (err) {
  ...
  connect(callbacks: LiveDataCallbacks = {}): void {
    console.log('🔌 LiveDataService.connect() called with callbacks:', Object.keys(callbacks));
  ...
  private connectMarketData(): void {
    ...
    try {
      ...
      this.marketClient.onOpen(() => {
        ...
        this.subscribeToSymbols(['AAPL', 'MSFT', 'GOOGL', 'TSLA', 'NVDA', 'SPY']);
      });
      ...
      this.marketClient.onMessage((event) => {
        ...
        try {
          ...
          if (message.type === 'market_data' || message.type === 'quote_update') {
  ...
  private connectNewsStream(): void {
    ...
    try {
      this.newsClient = new WSClient('/ws/news', { baseUrl: this.baseUrl, maxReconnectAttempts: this.maxReconnectAttempts });
        ...
        try {
          ...
          if (message.type === 'news_update') {
            ...
            const newsItem: NewsItem = {
              ...
              published_date: (data?.published || new Date().toISOString()) as string,
  ...
  private connectAlerts(): void {
    try {
      this.alertsClient = new WSClient('/ws/alerts', { baseUrl: this.baseUrl, maxReconnectAttempts: this.maxReconnectAttempts });
        ...
        try {
          ...
          if (message.type === 'alert_triggered') {
            ...
            const alert: Alert = {
              ...
              type: (alertData?.type || 'price') as 'price' | 'volume' | 'news' | 'technical',
              symbol: (alertData?.symbol || '') as string,
              condition: (alertData?.condition || '') as string,
              target_value: (alertData?.target_value || alertData?.value || 0) as number,
  ...
  private connectSignals(): void {
    try {
      this.signalsClient = new WSClient('/ws/signals', { baseUrl: this.baseUrl, maxReconnectAttempts: this.maxReconnectAttempts });
        ...
        try {
          ...
          if (message.type === 'signal_generated') {
            ...
            const signal: TradingSignal = {
              ...
              signal_type: (data?.action || data?.signal_type || 'HOLD') as 'BUY' | 'SELL' | 'HOLD',
  ...
  private connectPortfolio(): void {
    try {
      this.portfolioClient = new WSClient('/ws/portfolio', { baseUrl: this.baseUrl, maxReconnectAttempts: this.maxReconnectAttempts });
      ...
      this.portfolioClient.onMessage((event) => {
        try {
          ...
          if (message.type === 'portfolio_update' || message.type === 'portfolio_snapshot') {
            const data = message.data as Record<string, unknown>;
            const portfolioData = data?.portfolio as Record<string, unknown>;
            const positionsData = data?.positions as Array<Record<string, unknown>>;
            ...
            if (portfolioData) {
              const portfolio: Portfolio = {
                ...
                daily_pnl_percent: (portfolioData.daily_pnl_percent || 0) as number,
                positions: []
              };
  ...
  private connectCharts(): void {
    try {
      this.chartsClient = new WSClient('/ws/charts', { baseUrl: this.baseUrl, maxReconnectAttempts: this.maxReconnectAttempts });
  ...
  private connectSentiment(): void {
    try {
      this.sentimentClient = new WSClient('/ws/sentiment', { baseUrl: this.baseUrl, maxReconnectAttempts: this.maxReconnectAttempts });
  ...
  disconnect(): void {
    ...
    [this.marketClient, this.newsClient, this.alertsClient, this.signalsClient, this.portfolioClient, this.chartsClient, this.sentimentClient].forEach(c => c?.disconnect());
  ...
  sendChartCommand(message: Record<string, unknown>): void {
    if (this.chartsClient) {
      ...
      const action = (message as Record<string, unknown>)?.action as string | undefined;
