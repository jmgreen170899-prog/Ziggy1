...
class APIClient {
  ...
  async queryRAG(request: RAGQueryRequest): Promise<RAGQueryResponse> {
    const response = await this.client.post<RAGQueryResponse>(
      "/query",
      request,
    );
  ...
  async getQuote(symbol: string): Promise<Quote> {
    ...
    }>(`/market/overview?symbols=${symbol}`);
    const symbolData = response.data.symbols?.[symbol] as Record<
      string,
      unknown
    >;
    if (!symbolData) {
    ...
    const quote: Quote = {
      ...
      change_percent:
        (((symbolData.chg1d as number) || 0) /
          ((symbolData.ref as number) || 1)) *
        100,
  ...
  async getMultipleQuotes(symbols: string[]): Promise<Quote[]> {
    ...
    const response = await this.client.get<{
      symbols: Record<string, unknown>;
    }>(`/market/overview?symbols=${symbolsParam}`);
  ...
  async getMarketSectors(): Promise<
    Array<{ name: string; change_percent: number; leaders: string[] }>
  > {
      ...
      try {
        ...
        const avgChange =
          validQuotes.length > 0
            ? validQuotes.reduce(
                (sum: number, quote: Quote) =>
                  sum + (quote.change_percent || 0),
                0,
              ) / validQuotes.length
  ...
  async getTradingSignals(): Promise<TradingSignal[]> {
    ...
    try {
      ...
      const mapped = Object.entries(data.signals || {})
        .map(([ticker, entry]) =>
          entry?.signal
            ? ({ ...entry.signal, symbol: ticker } as TradingSignal)
            : null,
        )
  ...
  async getPortfolio(): Promise<Portfolio> {
    ...
    const [portfolioResponse, positionsResponse] = await Promise.all([
      ...
      this.client.get<{ positions: Record<string, unknown>[] }>(
        "/trade/positions",
      ),
    ]);
    ...
    const totalPositionValue = positions.reduce(
      (sum: number, pos: Record<string, unknown>) =>
        sum + ((pos.market_value as number) || 0),
      0,
    );
    const cash_balance = Math.max(
      0,
      portfolioSummary.total_value - totalPositionValue,
    );
    ...
    const formattedPositions = positions.map(
      (pos: Record<string, unknown>) => ({
        ...
        current_price:
          (pos.current_price as number) || (pos.avg_price as number) || 0,
  ...
  > {
    try {
      ...
      return orders.map((order: Record<string, unknown>) => ({
        ...
        type:
          (order.side as string)?.toUpperCase() === "BUY"
            ? ("BUY" as const)
            : ("SELL" as const),
        quantity: (order.quantity as number) || (order.qty as number) || 0,
        entry_price:
          (order.filled_avg_price as number) ||
          (order.limit_price as number) ||
          0,
        exit_price:
          (order.status as string) === "filled"
            ? (order.filled_avg_price as number)
            : null,
        ...
        closed_at:
          (order.status as string) !== "open"
            ? (order.filled_at as string) || (order.updated_at as string)
            : null,
      }));
    } catch (error) {
  ...
  async runScreener(criteria: Record<string, unknown>): Promise<Quote[]> {
    const response = await this.client.get("/trade/screener", {
      params: criteria,
    });
    return response.data;
  }
  ...
  async getNews(): Promise<NewsItem[]> {
    const response =
      await this.client.get<BackendNewsResponse>("/news/headlines");
    return this.transformNewsItems(response.data.items || []);
  }

  async getNewsForSymbol(symbol: string): Promise<NewsItem[]> {
    const response = await this.client.get<BackendNewsResponse>(
      `/news/headlines?symbol=${symbol}`,
    );
    return this.transformNewsItems(response.data.items || []);
  }
  ...
  private transformNewsItems(items: BackendNewsItem[]): NewsItem[] {
    return items.map((item: BackendNewsItem) => ({
      ...
      source: item.source || "",
      sentiment:
        (item.label as "positive" | "negative" | "neutral") || "neutral",
  ...
  async getNewsSentiment(
    symbol?: string,
  ): Promise<{ sentiment: string; score: number; symbols: string[] }> {
      ...
      for (const symbol of symbols) {
        ...
        if (data && data.price) {
          ...
          cryptoPrices.push({
            ...
            price: data.price,
            change_24h: data.change_pct_24h
              ? (data.price * data.change_pct_24h) / 100
              : 0,
  ...
  async getCryptoAnalysis(
    symbol: string,
  ): Promise<{ technical: Record<string, unknown>; sentiment: string }> {
  ...
  async getAlerts(): Promise<Alert[]> {
    const response =
      await this.client.get<BackendAlertResponse>("/alerts/list");
  ...
  async updateAlert(id: string, alert: Partial<Alert>): Promise<Alert> {
    ...
    const response = await this.client.post<Alert>("/alerts/create", {
      ...alert,
      id,
    });
    return response.data;
  ...
  async getAdaptationMetrics(): Promise<AdaptationMetrics> {
    const response =
      await this.client.get<AdaptationMetrics>("/learning/status");
    return response.data;
  }
  ...
  async submitAnonymizedTradeData(
    submission: TradeDataSubmission,
  ): Promise<ZiggyBrainLearning> {
    ...
    const response = await this.client.post<ZiggyBrainLearning>(
      "/learning/run",
      submission,
    );
    return response.data;
  ...
  async updateTradePrivacySettings(
    settings: TradeDataPrivacySettings,
  ): Promise<void> {
  ...
  async getZiggyBrainInsights(): Promise<ZiggyBrainLearning> {
    ...
    const response =
      await this.client.get<ZiggyBrainLearning>("/learning/status");
    return response.data;
  }
  ...
  async runBacktest(params?: Record<string, unknown>): Promise<unknown> {
    const res = await this.client.post(
      "/trading/backtest",
      params || { symbols: ["AAPL"], strategy: "buy_hold", period_days: 1 },
    );
    return res.data;
  }
  ...
  createAnonymizedTradeData(
    ...
    return trades.map((trade) => {
      ...
      const sessionHash = btoa(
        sessionDate + Math.random().toString(),
      ).substring(0, 16);

      // Anonymize quantity into tiers
      const getQuantityTier = (
        qty: number,
        price: number,
      ...
      const getOutcomeTier = (
        outcome?: number,
      ): "loss" | "breakeven" | "small_gain" | "large_gain" | undefined => {
      ...
      return {
        ...
        time_bucket:
          new Date(trade.timestamp).toISOString().substring(0, 13) +
          ":00:00.000Z", // Round to hour
        strategy_type:
          (trade.strategy as
            | "momentum"
            | "mean_reversion"
        ...
        outcome_tier: userSettings.share_outcomes
          ? getOutcomeTier(trade.outcome)
          : undefined,
