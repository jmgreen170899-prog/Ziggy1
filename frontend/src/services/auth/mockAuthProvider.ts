...
export interface SecurityEvent {
  id: string;
  type: 'sign_in' | 'sign_out' | 'password_change' | 'totp_enable' | 'totp_disable' | 'device_revoke' | 'failed_login' | 'failed_signup' | 'password_reset_requested';
  timestamp: string;
  ip: string;
  userAgent: string;
  location?: string;
...
const generateToken = () => `mock_token_${Date.now()}_${Math.random().toString(36).substr(2, 16)}`;
...
const MOCK_LOCATIONS = ['New York, NY', 'London, UK', 'Tokyo, JP', 'Sydney, AU'];
...
const checkRateLimit = (action: string, limit: number): boolean => {
  const rateLimits = getStorageItem<RateLimitData>(STORAGE_KEYS.RATE_LIMITS, {});
  const key = `${action}_${Date.now()}`;
  const now = Date.now();
  
  // Clean old entries
  Object.keys(rateLimits).forEach(k => {
    if (now - parseInt(k.split('_').pop() || '0') > RATE_LIMITS.LOCKOUT_DURATION) {
      delete rateLimits[k];
    }
  });
  
  // Count recent attempts
  const recentAttempts = Object.keys(rateLimits)
...
const logSecurityEvent = (event: Omit<SecurityEvent, 'id' | 'timestamp' | 'ip' | 'userAgent' | 'location'>): void => {
...
export class MockAuthProvider {
  ...
  async verifyEmail({ email, code }: VerifyEmailRequest): Promise<{ user: User }> {
    await simulateDelay();
    
    const pending = getStorageItem<PendingVerification | null>(STORAGE_KEYS.PENDING_VERIFICATION, null);
  ...
  async signIn({ email, password, remember = false }: Omit<SignInRequest, 'totpCode'>): Promise<SignInResponse> {
    await simulateDelay();
    ...
    const adminEnabled = process.env.NEXT_PUBLIC_AUTH_ENABLE_ADMIN === 'true' || isDevMode;
    
    // Admin credentials check
    const isAdminEmail = email.toLowerCase() === 'admin@ziggyclean.com' || email.toLowerCase() === 'admin';
    const isAdminPassword = password === 'admin';
    
    // Dev user credentials check  
    const isDevEmail = email.toLowerCase() === 'user' || email.toLowerCase() === 'dev@localhost';
    ...
    if (adminEnabled && (
      ...
      if (!adminEnabled) {
        throw new Error('Authentication is currently disabled. Please contact the administrator.');
      } else {
    ...
    const user: User = (isDevEmail && isDevPassword) ? {
      id: 'dev-user-001',
      email: 'dev@localhost',
    ...
    const sessions = getStorageItem<DeviceSession[]>(STORAGE_KEYS.DEVICE_SESSIONS, []);
  ...
  async signOut(): Promise<void> {
    ...
    const deviceSessions = getStorageItem<DeviceSession[]>(STORAGE_KEYS.DEVICE_SESSIONS, []);
  ...
  async requestPasswordReset({ email }: PasswordResetRequest): Promise<{ ok: boolean }> {
    await simulateDelay();
    ...
    throw new Error('Password reset is disabled. Contact administrator for assistance.');
  }

  async resetPassword({ email, code, newPassword }: ResetPasswordRequest): Promise<{ ok: boolean }> {
    await simulateDelay();
    
    const resetData = getStorageItem<PasswordResetData | null>(`ziggy_password_reset_${email}`, null);
  ...
  async getSession(): Promise<{ user: User | null; accessToken?: string; refreshToken?: string }> {
    const user = getStorageItem<User | null>(STORAGE_KEYS.USER, null);
  ...
  async getDevices(): Promise<DeviceSession[]> {
    ...
    const allSessions = getStorageItem<DeviceSession[]>(STORAGE_KEYS.DEVICE_SESSIONS, []);
    return allSessions.filter(session => session.userId === user.id);
  }

  async revokeDevice({ sessionId }: { sessionId: string }): Promise<{ ok: boolean }> {
    await simulateDelay();
  ...
  async updateProfile({ name, avatarUrl, timezone }: UpdateProfileRequest): Promise<{ user: User }> {
    await simulateDelay();
  ...
  async updateEmail({ newEmail, code }: UpdateEmailRequest): Promise<{ user: User }> {
    await simulateDelay();
    ...
    const users = getStorageItem<User[]>('ziggy_mock_users', []);
    if (users.some(u => u.email.toLowerCase() === newEmail.toLowerCase() && u.id !== user.id)) {
  ...
  async updatePassword({ currentPassword, newPassword }: UpdatePasswordRequest): Promise<{ ok: boolean }> {
    await simulateDelay();
    ...
    const storedPasswordHash = getStorageItem(`ziggy_mock_password_${user.id}`, '');
