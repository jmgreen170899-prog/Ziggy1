...
const generateToken = () => `local_token_${Date.now()}_${Math.random().toString(36).substr(2, 16)}`;
...
export class LocalAuthProvider {
  ...
  async requestPasswordReset({ email }: PasswordResetRequest): Promise<{ ok: true }> {
    await simulateDelay();
  ...
  async resetPassword({ email, code, newPassword }: ResetPasswordRequest): Promise<{ ok: true }> {
    await simulateDelay();
    
    const resetData = getStorageItem<PasswordResetData | null>(`ziggy_local_password_reset_${email}`, null);
  ...
  async enableTotp(): Promise<TotpSetupResponse> {
    ...
    const recoveryCodes = Array.from({ length: 8 }, () => Math.random().toString(36).substring(2, 11));
  ...
  async verifyTotp({ code }: { code: string }): Promise<{ ok: true }> {
    ...
    const totpData = getStorageItem<{ secret: string; recoveryCodes: string[] } | null>(STORAGE_KEYS.TOTP_SECRETS, null);
    if (!totpData) {
  ...
  async updateProfile(data: UpdateProfileRequest): Promise<{ ok: true; user: User }> {
    await simulateDelay();
  ...
  async updateEmail(data: UpdateEmailRequest): Promise<{ ok: true; user: User }> {
    await simulateDelay();
  ...
  async updatePassword(data: UpdatePasswordRequest): Promise<{ ok: true }> {
    ...
    const storedPasswordHash = getStorageItem(`ziggy_local_password_${user.id}`, '');
  ...
  async revokeDevice({ sessionId }: { sessionId: string }): Promise<{ ok: true }> {
    await simulateDelay(50, 150);
    const sessions = getStorageItem<DeviceSession[]>(STORAGE_KEYS.DEVICE_SESSIONS, []);
    const filtered = sessions.filter(s => s.id !== sessionId);
    setStorageItem(STORAGE_KEYS.DEVICE_SESSIONS, filtered);
    logSecurityEvent({ type: 'device_revoke', success: true, details: `Revoked ${sessionId}` });
    return { ok: true };
  }
  ...
  async refresh(): Promise<{ accessToken: string; refreshToken: string }> {
    ...
    const newTokens = { accessToken: generateToken(), refreshToken: generateToken() };
    setStorageItem(STORAGE_KEYS.TOKENS, { ...tokens, ...newTokens, expiresAt: Date.now() + 24*60*60*1000 });
    return newTokens;
  }
