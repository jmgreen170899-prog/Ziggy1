...
const BACKEND_BASE = process.env.BACKEND_BASE?.replace(/\/$/, '') || 'http://127.0.0.1:8000'
...
function buildBackendUrl(req: Request, backendPath: string): string {
  ...
  const target = new URL(BACKEND_BASE + (backendPath.startsWith('/') ? backendPath : `/${backendPath}`))
  // Copy query params
...
export async function proxyJson<T = unknown>(
  ...
  try {
    ...
    if (res.ok) {
      return NextResponse.json<ProxyEnvelope<T>>({ ok: true, status: 'ok', data: data as T })
    }
    ...
    return NextResponse.json<ProxyEnvelope<T>>({
      ...
      errors: [{ path: backendPath, status: res.status, message: typeof data === 'string' ? data : undefined }],
      data: isJson ? (data as T) : undefined,
    })
  } catch (e) {
    const msg = (e instanceof Error ? e.message : String(e)) || 'request_failed'
    const reason = /abort|timed out|timeout/i.test(msg) ? 'timeout' : 'network_error'
    return NextResponse.json<ProxyEnvelope<T>>({ ok: false, status: 'degraded', reason, errors: [{ path: backendPath, message: msg }] })
  }
}

// Helper for batch OHLC to compute mixed status
export function mixedStatusFromBatch(batch: { summary?: { failed?: number } } | null | undefined): Status {
...
export function degraded<T>(reason: string, data?: T, errors?: ProxyEnvelope<T>['errors']) {
  return NextResponse.json<ProxyEnvelope<T>>({ ok: false, status: 'degraded', reason, data, errors })
}
export function mixed<T>(data: T) {
  return NextResponse.json<ProxyEnvelope<T>>({ ok: true, status: 'mixed', data })
}
