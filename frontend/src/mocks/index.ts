...
class MockServiceAdapter {
  ...
  async createTradingPlan(
    plan: Omit<TradingPlan, "id" | "createdAt">,
  ): Promise<TradingPlan> {
    if (useMocks) {
      await simulateApiDelay();
  ...
  async runScenario(params: {
    ...
    if (useMocks) {
      ...
      const weightedPnL = scenarios.reduce(
        (sum, s) => sum + (s.pnl * s.probability) / 100,
        0,
      );
