...
export const mockEvidence: Evidence[] = [
  ...
  },
  {
    ...
    headlines: [
      ...
      {
        title: 'EV Competition Intensifies as Traditional Automakers Gain Ground',
...
export function getEvidenceByPrediction(predictionId: string): Evidence | undefined {
  return mockEvidence.find(evidence => evidence.predictionId === predictionId);
}

export function getBullishEvidence(): Evidence[] {
  return mockEvidence.filter(evidence => evidence.sentiment.label === 'bullish');
}

export function getBearishEvidence(): Evidence[] {
  return mockEvidence.filter(evidence => evidence.sentiment.label === 'bearish');
}

export function getHighConfidenceEvidence(minConfidence: number = 80): Evidence[] {
  return mockEvidence.filter(evidence => evidence.sentiment.confidence >= minConfidence);
}
...
export function getStrongPatterns(minStrength: number = 80): Evidence[] {
  return mockEvidence.filter(evidence => 
    evidence.patterns.strength >= minStrength
  );
}
...
export function calculateSentimentTrend(evidence: Evidence[]): {
  ...
  const trend = diff > 0.1 ? 'improving' : diff < -0.1 ? 'deteriorating' : 'stable';
  
  const avgConfidence = evidence.reduce((sum, e) => sum + e.sentiment.confidence, 0) / evidence.length;
