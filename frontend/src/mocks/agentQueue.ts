...
export const mockAgentQueue: AgentAction[] = [
  ...
  },
  {
    ...
    requiresApproval: false,
    result: 'Technical analysis complete. Bear flag pattern identified with 74% confidence.'
  },
  {
    id: 'agent_007',
    type: 'monitor',
    ...
    requiresApproval: false,
    error: 'Market data feed timeout. Unable to establish monitoring connection.'
  },
  {
    id: 'agent_008',
    type: 'alert',
...
export function getActionsByStatus(status: AgentAction['status']): AgentAction[] {
...
export function getActionsByPriority(priority: AgentAction['priority']): AgentAction[] {
...
export function getPendingActions(): AgentAction[] {
  return mockAgentQueue.filter(action => 
    action.status === 'pending' || action.status === 'in_progress'
  );
}

export function getActionsRequiringApproval(): AgentAction[] {
  return mockAgentQueue.filter(action => action.status === 'requires_approval');
}
...
export function getFailedActions(hours: number = 24): AgentAction[] {
  ...
  return mockAgentQueue.filter(action => 
    ...
    new Date(action.createdAt) > cutoff
  );
}
...
export function getEstimatedCompletionTime(): Date | null {
  const inProgress = mockAgentQueue.filter(action => 
    action.status === 'in_progress' && action.eta
  );
  
  if (inProgress.length === 0) return null;
  
  const latestEta = Math.max(...inProgress.map(action => 
...
export function approveAction(actionId: string, approvedBy: string): AgentAction | null {
...
export function completeAction(actionId: string, result?: string): AgentAction | null {
  const action = mockAgentQueue.find(a => a.id === actionId);
