...
const ROUTES = [
  ...
  { name: 'chat', path: '/chat', dataSelector: '[data-testid="chat-interface"]' },
  { name: 'market', path: '/market', dataSelector: '[data-testid="market-data"]' },
  { name: 'alerts', path: '/alerts', dataSelector: '[data-testid="alerts-list"]' },
  { name: 'paper-trading', path: '/paper', dataSelector: '[data-testid="paper-trading"]' },
  { name: 'portfolio', path: '/portfolio', dataSelector: '[data-testid="portfolio"]' },
  { name: 'settings', path: '/settings', dataSelector: '[data-testid="settings"]' },
  { name: 'status', path: '/status', dataSelector: '[data-testid="status-page"]' },
];
...
async function analyzePageContent(page: Page): Promise<RouteAuditResult['metrics']> {
  const metrics = await page.evaluate(() => {
    ...
    // Count tables
    const tableCount = document.querySelectorAll('table, [role="table"], [data-testid*="table"]').length;
    ...
    const missingIndicators = document.querySelectorAll('[data-testid*="missing"], .text-gray-400, .opacity-50');
    missingIndicators.forEach((el, index) => {
      const text = el.textContent?.trim();
      if (text && (text.includes('--') || text.includes('N/A') || text.includes('Loading') || text === '')) {
        missingFields.push(`Element ${index}: ${text}`);
      }
    });

    // Count stale badges or TTL indicators
    const staleBadges = document.querySelectorAll('[data-testid*="stale"], [class*="stale"], .text-red-500').length;
...
test.describe('UI Audit - Route Health Check', () => {
  ...
  test.afterAll(async () => {
    ...
    const summary = {
      ...
      successfulRoutes: auditResults.filter(r => r.status === 'success').length,
      failedRoutes: auditResults.filter(r => r.status === 'error').length,
      timeoutRoutes: auditResults.filter(r => r.status === 'timeout').length,
      totalIssues: auditResults.reduce((sum, r) => 
        sum + r.metrics.nanCells + r.metrics.infinityCells + r.metrics.missingFields.length + 
        r.metrics.consoleErrors.length + r.metrics.networkErrors.length, 0
      ),
      routes: auditResults.map(r => ({
        name: r.route,
        status: r.status,
        issues: r.metrics.nanCells + r.metrics.infinityCells + r.metrics.missingFields.length + 
                r.metrics.consoleErrors.length + r.metrics.networkErrors.length
      }))
    };
    
    await fs.writeFile(path.join(artifactsDir, 'ui_audit_summary.json'), JSON.stringify(summary, null, 2));
  });
    ...
    test(`Route: ${route.name} (${route.path})`, async ({ page }) => {
      ...
      try {
        ...
        try {
          ...
          console.warn(`Data selector not found for ${route.name}: ${route.dataSelector}`);
        }
        ...
        try {
          const screenshotPath = path.join(artifactsDir, `error_${result.screenshot}`);
