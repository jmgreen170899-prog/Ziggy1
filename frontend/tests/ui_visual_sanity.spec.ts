...
test.describe("Visual Sanity - Loading States", () => {
  test("Pages show loading indicators appropriately", async ({ page }) => {
    ...
    const hasSkeletons =
      (await page.locator(".skeleton, .animate-pulse").count()) > 0;
...
test.describe("Visual Sanity - Error States", () => {
  ...
  test("Error states have retry actions", async ({ page }) => {
    ...
    if (errorText > 0) {
      const tryAgainButton = await page
        .locator("button:has-text(/try again|retry|reload/i)")
        .count();
...
test.describe("Visual Sanity - Empty States", () => {
  test("Empty states render with proper messaging", async ({ page }) => {
    ...
    const emptyState = await page
      .locator("text=/no.*found|empty|get started/i")
      .count();
  ...
  test("Empty states include visual icons or illustrations", async ({
    page,
  }) => {
    await page.goto("/news");
    ...
    const emptyMessage = await page
      .locator("text=/no.*articles|empty/i")
      .count();
...
test.describe("Visual Sanity - Component Consistency", () => {
  ...
  test("Buttons use consistent styling", async ({ page }) => {
    ...
    if (buttons > 0) {
      ...
      const styledButtons = await page
