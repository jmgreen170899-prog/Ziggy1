...
class RepoIntegrityChecker:
    ...
    def check_duplicate_dependencies(self) -> List[str]:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if '==' in line:
                    ...
                    if package in seen_packages:
                        duplicates.append(f"{package} (lines {seen_packages[package]} and {i+1})")
                    else:
            ...
            if duplicates and self.fix_issues:
                ...
                print(f"✅ Fixed {len(duplicates)} duplicate dependencies in requirements.lock")
    ...
    def check_stray_test_files(self) -> List[str]:
        ...
        if stray_tests and self.fix_issues:
            ...
            for stray_test in stray_tests[:]:  # Copy list to avoid modification during iteration
                        ...
                        for line in content.split('\n'):
                            if line.strip().startswith('testpaths'):
                                if 'tests' not in line:
                                    config_issues.append("pytest.ini testpaths should include 'tests'")
                    else:
                        config_issues.append("pytest.ini missing testpaths configuration")
            except Exception as e:
                ...
                with open(pyproject_toml, 'r', encoding='utf-8') as f:
                    ...
                    if '[tool.pytest.ini_options]' in content:
                        if 'testpaths = ["tests"]' not in content:
                            config_issues.append("pyproject.toml pytest testpaths should be ['tests']")
            except Exception as e:
