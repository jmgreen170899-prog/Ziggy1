...
async function analyzeDataQuality(
  page: Page,
): Promise<TabAuditData["dataMetrics"]> {
  return await page.evaluate((selectors) => {
    ...
    const textElements = document.querySelectorAll(
      selectors.prices + ", " + selectors.percentages,
    );
...
async function analyzeLayoutIssues(
  page: Page,
): Promise<TabAuditData["layoutIssues"]> {
    ...
    if (window.innerWidth < 768) {
      ...
      smallElements.forEach((el) => {
        ...
        if (rect.width < 44 || rect.height < 44) {
          issues.responsiveIssues.push(
            `Touch target too small: ${el.tagName}.${el.className}`,
          );
        }
      });
    }
...
test.describe("ZiggyAI UI Audit", () => {
  ...
  for (const route of ROUTES_TO_AUDIT) {
    test(`Audit ${route.name} tab`, async ({ page, browser }) => {
      ...
      auditReport.environment.userAgent = await page.evaluate(
        () => navigator.userAgent,
      );
      ...
      try {
        ...
        console.log(
          `✅ Audited ${route.name}: ${tabData.dataMetrics.dataCards} cards, ${tabData.dataMetrics.dataRows} rows, ${tabData.consoleErrors.length} errors`,
        );
      } catch (error) {
  ...
  test.afterAll(async () => {
    ...
    auditReport.summary.criticalIssues = auditReport.tabs.reduce(
      (total, tab) => {
        return (
          total +
          tab.dataMetrics.nanValues.length +
