...
class MobileAuthRequest(BaseModel):
    ...
    device_name: str = Field(default="Android Device", description="Human-readable device name")
...
class MobileMarketSnapshot(BaseModel):
    ...
    cache_ttl: int = Field(default=60, description="Seconds until data should be refreshed")
...
class MobileDeviceInfo(BaseModel):
    ...
    device_id: str
    device_name: str
    push_token: str | None = Field(default=None, description="FCM push notification token")
...
async def mobile_sync(
    since: int | None = Query(default=None, description="Unix timestamp for incremental sync"),
    include: str = Query(default="all", description="Comma-separated: quotes,signals,alerts,news,portfolio"),
    current_user: dict[str, Any] = Depends(get_current_user)
):
    """
