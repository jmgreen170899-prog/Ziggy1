...
const RE_FETCH = /(fetch\s*\(\s*(["'`])([^\2\n\r]+)\2\s*(?:,\s*\{[\s\S]*?method\s*:\s*(["'`])([A-Za-z]+)\3[\s\S]*?\})?\s*\))/g;
const RE_AXIOS = /(axios\.(get|post|put|patch|delete|head|options)\s*\(\s*(["'`])([^\3\n\r]+)\3)/g;
...
function detectVia(url: string): 'proxy'|'direct'|'relative' {
  ...
  if (/^https?:\/\/127\.0\.0\.1:8000\//.test(url) || /^https?:\/\/localhost:8000\//.test(url)) return 'direct';
...
function normalizeRouteFromFile(file: string): string | null {
  ...
  if (rel.startsWith('src/app/')) {
    ...
    const routePath = '/' + withoutFile.map(s => {
      if (s.startsWith('(') && s.endsWith(')')) return ''; // route group
      if (s === 'index') return '';
      if (s.startsWith('[') && s.endsWith(']')) return s.replace(/\[|\]/g, ':'); // dynamic -> :param
      return s;
    }).filter(Boolean).join('/');
    return routePath || '/';
  }
  ...
  if (rel.startsWith('pages/')) {
    ...
    route = '/' + route.replace(/index$/, '').replace(/\[\w+?\]/g, m => ':' + m.slice(1, -1));
...
function scanFileForCalls(file: string) {
  ...
  const calls: Array<{method: string; url: string; file: string; line: number; via: 'direct'|'proxy'|'relative'}> = [];
  const add = (method: string, url: string, index: number) => {
    const pre = src.slice(0, index);
    const line = pre.split(/\r?\n/).length;
    calls.push({ method: method.toUpperCase(), url, file: toPosix(file), line, via: detectVia(url) });
  };
...
async function scanFrontendCalls() {
  ...
  for (const file of files) {
    try { allCalls.push(scanFileForCalls(file)); } catch {}
  }
  return allCalls.flat();
}

function groupCallsByRoute(routes: string[], calls: ReturnType<typeof scanFileForCalls>) {
  const map: Record<string, Array<{method: string; url: string; file: string; line: number; via: 'direct'|'proxy'|'relative'}>> = {};
  ...
  for (const c of calls) {
    ...
    for (const r of routes) {
      ...
      if (score > maxMatch) { maxMatch = score; picked = r; }
    }
    if (!picked) picked = '/';
    map[picked].push(c);
  }
