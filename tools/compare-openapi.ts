...
async function main() {
  ...
  if (!fs.existsSync(IN_FILE)) {
    console.error(
      `Missing input file: ${IN_FILE}. Run 'npm run audit:scan' first.`,
    );
    process.exit(1);
  }
  const scan = JSON.parse(fs.readFileSync(IN_FILE, "utf8"));
  const calls = Object.values(scan.byRoute || {}).flat() as Array<{
    method: string;
    url: string;
    file: string;
    line: number;
  ...
  for (const c of normalized) {
    ...
    const isBackendPath =
      kind === "backend" || (kind === "proxy" && backendAvailable);

    if (!backendAvailable) {
      // Cannot verify against OpenAPI; mark as unknown
      issues.push({
        type: "backend-unavailable",
        message: `OpenAPI not reachable while checking ${method} ${pth}`,
        method,
        path: pth,
        file: toPosix(c.file),
        line: c.line,
    ...
    if (kind === "relative") {
      ...
      issues.push({
        type: "relative-url",
        message: `Relative URL encountered: ${method} ${pth}`,
        method,
        path: pth,
        file: toPosix(c.file),
        line: c.line,
    ...
    if (!foundPath || !methodsForPath) {
      issues.push({
        ...
        message: `No OpenAPI path for ${method} ${pth}`,
        method,
        path: pth,
        file: toPosix(c.file),
        line: c.line,
      ...
      issues.push({
        type: "wrong-method",
        message: `Method mismatch for ${method} ${pth} (OpenAPI allows: ${methodsForPath.join(",")})`,
        method,
        path: pth,
        file: toPosix(c.file),
        line: c.line,
  ...
  const result = {
    ...
    issues,
    openapiSummary: backendAvailable
      ? { paths: Object.keys(apiPaths).length }
      : { paths: 0 },
  };
