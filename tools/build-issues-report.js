...
function summarizeArtifacts() {
  ...
  return files.map((f) => ({
    file: f,
    path: path.join("frontend/tests/artifacts", f),
  }));
}
...
function main() {
  ...
  for (const route of scan.routes || []) {
    ...
    const statusSummaries = calls.map(
      (c) => `- ${c.method} ${c.url} (${c.via}) @ ${c.file}:${c.line}`,
    );
    ...
    const routeLines = [
      `- Route: ${route}`,
      ...(statusSummaries.length
        ? statusSummaries
        : ["- No API calls detected on this route."]),
      ...(routeIssues.length
        ? [
            "- Detected mismatches:",
            ...routeIssues.map((i) => `  - [${i.type}] ${i.message}`),
          ]
        : []),
      ...repro,
    ];
  ...
  // Artifacts
  lines.push(
    buildSection("Artifacts", [
      ...(artifacts.length
        ? artifacts.map((a) => `- ${a.path}`)
        : ["- No artifacts yet. Run the smoke test."]),
    ]),
  );
